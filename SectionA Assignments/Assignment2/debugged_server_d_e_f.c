...
void print_process_details(int pid, long utime, long stime, const char *name) {
    printf("PID: %d, User Time: %ld, System Time: %ld, Name: %s\n", pid, utime, stime, name);
}
...
int scan_process_file(int pid, info_list **process_info) 
{
    ...
    fclose(file);
    print_process_details(pid, utime, stime, name);
