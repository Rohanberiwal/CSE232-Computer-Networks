...
void *client_thread(void *arg) 
{
    ...
    // scrpy is used to copy the whole string to teh  var 
    strcpy(message, "Hello from client thread ");
    char thread_id_str[20];
    // s print is a fucntion that is simialt ot snsprint that is uwse to print the thread id and all infromtion about the thread
    sprintf(thread_id_str, "%ld", thread_id);
    // strcat concatenater the two strings 
