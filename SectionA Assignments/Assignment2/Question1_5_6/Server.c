...
void handle_client(int client_fd) 
{
    ...
    char response[BUFFER_SIZE] = "Top Two Processes by Total CPU Time:\n";
    ...
    for (int i = 0; i < 2 && i < process_count; i++)
    {
        ...
        {
            char temp[256];
            snprintf(temp, sizeof(temp), "PID: %d, Name: %s, Total CPU Time: %.6f seconds\n",
                     process_array[i]->pid, process_array[i]->name,
                     process_array[i]->total_cpu_time);
            ...
            for (int j = 0; temp[j] != '\0'; j++) 
            {
                ...
                { 
                    response[response_length++] = temp[j];
                }
            }
